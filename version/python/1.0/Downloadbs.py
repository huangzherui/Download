...
path = f'C:/Users/{username}/AppData/Local/ZHMarketDownload/{filename}/'
...
if not mode == 'open':
    ...
    #检测文件夹
    if not os.path.exists(f'C:/Users/{username}/AppData/Local/ZHMarketDownload'):
        os.makedirs(f'C:/Users/{username}/AppData/Local/ZHMarketDownload')
    if not os.path.exists(path):
        os.makedirs(path) 

    urldownload('http://github.com/huangzherui/Download/raw/zbhedit/zip/7z.dll',f'{path}7z.dll')
    urldownload('http://github.com/huangzherui/Download/raw/zbhedit/zip/7z.exe',f'{path}7z.exe')
    ...
    for i in range(programlist[AID]["spilttotal"]):
        ...
        urldownload(down_url,f"{path}{filename}.7z.{s}")
    os.system(f'powershell cd {path};./7z.exe x {filename}.7z.001')
    for i in range(programlist[AID]["spilttotal"]):
        s = '%03d' % int(i+1)
        os.system(f'powershell del {path}{filename}.7z.{s}')
os.system(f'powershell cd {path};start {path}{filename}.exe')